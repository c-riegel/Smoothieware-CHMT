                    ...
                    if(gcode->has_m) {
                        switch (gcode->m) {
                            ...
                            case 115: { // M115 Get firmware version and capabilities
                                ...
                                new_message.stream->printf("FIRMWARE_NAME:Smoothieware, FIRMWARE_URL:https%%3A//github.com/c-riegel/Smoothieware-CHMT, X-SOURCE_CODE_URL:https://github.com/c-riegel/Smoothieware-CHMT, FIRMWARE_VERSION:%s, X-FIRMWARE_BUILD_DATE:%s, X-SYSTEM_CLOCK:%ldMHz, X-AXES:%d, X-GRBL_MODE:%d", vers.get_build(), vers.get_build_date(), SystemCoreClock / 1000000, MAX_ROBOT_ACTUATORS, THEKERNEL->is_grbl_mode());
